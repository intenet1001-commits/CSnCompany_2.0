...
```bash
/tdd-plan "사용자 인증 시스템 (이메일+비밀번호, JWT)"
/tdd-plan --lang typescript "장바구니 도메인"
/tdd-plan --output docs/plans "결제 처리 유스케이스"
/tdd-plan --lang python --output plans "이메일 발송 서비스"
```
...
자세한 실행 프로토콜은 이 플러그인의 `skills/tdd-clean-planner/SKILL.md`를 참조하세요.
