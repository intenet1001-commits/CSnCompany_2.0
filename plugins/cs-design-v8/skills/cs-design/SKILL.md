...
### 5. Karpathy 원칙 — Think Before Designing (설계 전 가정 명시) (2026-04-20)

- **상황**: design-lead가 코드베이스를 읽고 즉시 디자인 평가를 시작. 대상 사용자나 브랜드 방향에 대한 암묵적 가정 하에 진행되어 방향이 틀리는 경우 발생.
- **발견**: Karpathy의 "Think Before Coding" 원칙: 해석이 여러 가지인 경우 하나를 선택하지 말고 모두 명시. "더 단순한 대안은?"을 먼저 질문. 디자인도 마찬가지로 tradeoff를 먼저 surface 해야 함.
- **교훈**: design-lead Step 0에 Assumption Surface 추가: "이 컴포넌트의 대상 사용자는 A인가 B인가? 단순화된 버전(variant 1)과 풍부한 버전(variant 2) 중 어느 방향인가?" AskUserQuestion 1회 후 진행.

### 6. gstack Dual-Mode — Quick Scan vs Deep Audit (2026-04-20)

- **상황**: 매번 5개 에이전트 풀 실행은 작은 UI 변경에 과도. 빠른 확인이 필요한 경우 전체 팀 구동이 비효율적.
- **발견**: gstack `/design-review`는 두 모드를 지원: Quick(git diff 파일만) vs Deep(전체 codebase). 작은 PR에는 Quick, 릴리즈 전에는 Deep.
- **교훈**: cs-design에 `--quick` 플래그 추가: 수정된 파일(git diff)만 대상으로 anti-pattern-detector와 visual-hierarchy만 실행. 기본값은 기존 5-agent full audit.

### 7. OMC Model Tier Assignment — 디자인 에이전트별 모델 할당 (2026-04-21)

- **상황**: cs-design의 5개 에이전트가 모두 동일 모델 사용. anti-pattern-detector(패턴 스캔, 단순)와 design-lead(전략 판단, 복잡)에 같은 모델을 쓰는 것은 비효율.
- **발견**: oh-my-claudecode와 plugins-for-claude-natives 모두 에이전트 frontmatter에 모델 명시. Haiku는 단순 스캔, Opus는 전략 판단에 할당.
- **교훈**: agent .md 파일에 `model:` 필드 추가. anti-pattern-detector/responsive-accessibility → haiku, visual-hierarchy/interaction-quality → sonnet, design-lead → opus.

### 8. plugins-for-claude-natives Agent-Council 패턴 — 디자인 결정 크로스 검증 (2026-04-21)

- **상황**: design-lead가 단일 관점으로 최종 디자인 등급을 결정. 모호한 케이스에서 "이 디자인이 정말 좋은가?" 두 번째 의견 부재.
- **발견**: plugins-for-claude-natives `agent-council`은 여러 AI(Gemini, GPT, Codex)에게 동일 질문을 던져 consensus를 추출. 동의 ≥5개, 논쟁 ≥3개 기준으로 신뢰도 판단.
- **교훈**: cs-design C등급 이하 판정 시 design-lead가 "steelman antithesis" 생성 추가: "이 디자인이 맞을 수 있는 최강의 이유는?" 반론을 찾아서 제시. 반론이 설득력 있으면 등급 재검토.
