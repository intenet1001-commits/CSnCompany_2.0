...
### 12. 컴포넌트 재작성 시 함수 스코프 & wrapper 누락 패턴 (2026-04-21)

- **상황**: renderV3Card 전체 재작성 후 빌드는 통과. 그러나 openLog(item) 호출이 컴포넌트 스코프에 없는 함수 참조 → 런타임 오류 (TypeScript는 암묵적 전역으로 통과).
- **발견**: 재작성 시 API 메서드 직접 호출(API.openLog(item.id))이 필요한데 wrapper 함수 이름만 복사하면 런타임에서만 실패. 또한 GitBranch 버튼을 setWorktreePickerState 직접 호출로 작성하면 openTmuxClaude wrapper가 포함하는 loadWorktrees, recordVisit 등 연계 로직이 모두 누락됨.
- **교훈**: 재작성 후 (1) 각 핸들러 함수가 실제로 스코프에 존재하는지 grep 확인, (2) UI 버튼→handler 매핑 시 wrapper 함수 vs 직접 API 호출 의도적으로 구분. 빌드 통과 ≠ 런타임 안전.

### 13. Supabase SELECT 컬럼 드리프트 — 빌드 통과 후 런타임 400 에러 (2026-04-21)
